...
public class LoginAdministrador {
	...
	public int validar_usuario(LoginAdministrador l){
		cx.con();
	...
	public LoginAdministrador getUsuario(LoginAdministrador la){		
		LoginAdministrador user = new LoginAdministrador();
		String com = "SELECT * FROM ADMINISTRADOR " +
				"WHERE NOMRE_USR='"+la.getNombre_usuario_usuario()+"' " +
						"AND PASSWORD_USR='"+la.getPassword_usuario()+"' " +
								"AND ESTADO_USR='1'";
		try {
			cx.con();
